...
function Player() {
  ...
  const firstBloods = allRounds.reduce((acc, round) => {
    const allKills =
      round.player_stats?.flatMap((p) => p.kill_events || []) || [];
  ...
  const flawlessRounds = allRounds.filter((round) => {
    ...
    const anyTeammmateDied = round.player_stats?.some(
      (p) =>
  ...
  const kastRounds = allRounds.filter((round) => {
    ...
    const myTeam = myStats.player_team;
    const myDeathEvent = round.player_stats
      ?.flatMap((p) => p.kill_events || [])
    ...
    const wasTraded =
      iDied && myDeathEvent
        ? round.player_stats
            ...
            .some(
              (k) =>
                round.player_stats?.find(
                  (ps) => ps.player_puuid === k.killer_puuid,
                )?.player_team === myTeam &&
  ...
  const damagePerRound =
    totalRounds > 0 ? Math.round(totalDamage / totalRounds) : 0;
  const killsPerRound =
  ...
  return (
    <div>
      ...
      <p>
        Região: {account.region.toUpperCase()} · Nível: {account.account_level}
      </p>

      <h2>Visão Geral</h2>

      <p>
        Rank Atual: {mmr.current_data?.currenttierpatched} —{" "}
        {mmr.current_data?.ranking_in_tier} RR
      </p>
      <p>
        Peak: {mmr.highest_rank?.patched_tier} ({mmr.highest_rank?.season})
      </p>
      <p>Vitórias: {wins}</p>
      <p>Derrotas: {losses}</p>
      <p>Win%: {winPercent}%</p>
      ...
      {matches.map((match) => {
        ...
        return (
          <div key={match.metadata.matchid}>
            <p>
              {match.metadata.map} — {me.character} —{me.stats.kills}/
              {me.stats.deaths}/{me.stats.assists} —
